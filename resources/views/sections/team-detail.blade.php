<x-layout :title="$team->name . ' | Azenion Teams'">
            ...
            <x-reveal class="rounded-[2rem] card-surface-soft p-8 sm:p-12 shadow-card backdrop-blur-xl">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
                    <div>
                        ...
                        <h1 class="mt-4 text-3xl font-semibold text-ink-50 sm:text-4xl">{{ $team->name }}</h1>
                        <p class="mt-2 text-sm text-ink-400">Created by {{ $team->owner->profile->full_name ?? $team->owner->name }}</p>
                    </div>
                    @auth
                        @if($team->owner_id !== auth()->id())
                            @php $isMember = $team->members->contains('user_id', auth()->id()); @endphp
                            @if($isMember)
                                <form action="{{ route('teams.leave', $team->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="rounded-full bg-red-500/10 border border-red-500/30 px-6 py-2.5 text-sm font-semibold text-red-400 hover:bg-red-500/20">Leave Team</button>
                                </form>
                            @else
                                <form action="{{ route('teams.join', $team->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="rounded-full bg-accent px-6 py-2.5 text-sm font-semibold text-white hover:bg-accent-400 shadow-glow">Join Team</button>
                                </form>
                            @endif
                        @endif
                    @endauth
                ...
                <div class="mt-8 border-t border-ink-800 pt-8">
                    <h2 class="text-lg font-semibold text-ink-50">About the Team</h2>
                    <p class="mt-3 text-base leading-relaxed text-ink-300">{{ $team->description ?? 'No description provided.' }}</p>
                </div>

                <div class="mt-10 border-t border-ink-800 pt-8">
                    <h2 class="text-lg font-semibold text-ink-50">Members ({{ $team->members->count() }})</h2>
